...
file write mf "\begin{tabular}{l|cccccc|cc|cc}" _n
...
forval ii=$i1/$i2 {
	...
	reg undeclared_frac `varlist' if `cc'&$include_cond&$ind_typenewcond, cluster(subj_id)
		forval iii=1/`n' {
			local ttt `: word `iii' of `tcond''
			local eee `: word `iii' of `eadd''
			test `ttt'
	...
	reg undeclared `varlist' if `cc'&$include_cond&$ind_typenewcond, cluster(subj_id)
		forval iii=1/`n' {
			local ttt `: word `iii' of `tcond''
			local eee `: word `iii' of `eadd''
			test `ttt'
	...
	esttab m1 m2 m3 m4 m5 using "$fname", drop(`varlist1') scalars(`scals') label mtitle("Maximal lying" "Partial lying" "Honest" "Fraction undeclared" "Amount undeclared") wide compress nonum title(`tt') `ap' fragment  prehead(&\multicolumn{6}{c|}{\bf `tt'}&\multicolumn{2}{c|}{\bf `tt'}&\multicolumn{2}{c}{\bf `tt'}\\ $preh) postfoot(\hline\hline) se star(* 0.10 ** 0.05 *** 0.01)
	}

file open mf using "$fname", write append
file write mf "\multicolumn{11}{p{18cm}}{\tiny `note'}\\" _n
